...
//写法四官方写法 这种方式可以获取json格式(正常可用)
   export const requestLogin = params => { console.log(params); 
  return $axios({url:'http://192.168.43.116:8080/park/login',
  	method:'post',
  	data:params,
   headers:{'Content-Type': 'application/x-www-form-urlencoded'}
}    //{headers:{'Access-Control-Allow-Origin': "*"}}
...
export const getPermission = params => { console.log(params); return $axios.get(`../../static/json/rolelist.js`,{ params: params }).then(res => res.data);};
  ...
  return $axios({url:'../../static/json/park.json',
   ...
   export const requestMenu = params => { console.log(params); 
    return $axios({url:'http://192.168.43.116:8080/park/treelist',
      method:'post',
      data:params,
     headers:{'Content-Type': 'application/x-www-form-urlencoded'}
  }    //{headers:{'Access-Control-Allow-Origin': "*"}}
